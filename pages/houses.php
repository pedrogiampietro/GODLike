...
if(empty($id))
{
	...
	if($houses_town > 0)
	{
		...
		foreach($houses_sql as $house)
		{
			...
			<td>'.$owner.'</td>
			<td><a href="index.php?subtopic=houses&show='.$house['id'].'" class="btn btn-primary">View</a></td>
			</tr>';
		}
		$main_content .= '</tbody></table><hr>';
	}

	$main_content .= '<br><form class="form-horizontal" role="form" action="?subtopic=houses" method="post">
	<fieldset>
...
else
{
	...
	</tbody></table><a href="?subtopic=houses" class="btn btn-primary form-control">Back</a></div></div>';
}
?>
