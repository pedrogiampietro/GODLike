...
if($logged) {
	...
	$main_content .= '
		...
		if($step == 1) {
			...
			$main_content .= '
				<form method="post" action="?subtopic=account&action=donate">
											...
											<tbody>
												<tr>
													<td>We inform the players and staff that '.$config['server']['serverName'].' has no financial interest. All income is obtained directly reapplied for server maintenance - this means that to make a donation, you are ensuring the stability and increasing quality.</br>
														...
														Check out <a href="?subtopic=account&action=manage#Products+Available">'.$config['server']['serverName'].' Shop</a> and see how you can make your Tibia Coins in the most profitable way to your situation.</br>
			...
			$main_content .= '
				<div class="SubmitButtonRow" >
					...
					<div class="RightButton" >
						<form method="post" action="?subtopic=account&action=manage" >
		...
		if($step == 2) {
			...
			<form method="post" action="?subtopic=account&action=donate"><div class="TableContainer" style="position:relative;">
				<div class="CaptionContainer">
				<div class="CaptionInnerContainer"> 
		...
		if($step == 3) {
			...
			if(!empty($donate_errors)) {
						...
						<TR>
							<TD ALIGN=center>
								<table border="0" cellspacing="0" cellpadding="0" >
									<form action="?subtopic=account&action=donate" method="post">
										<input type="hidden" name="step" value="2">
										<tr>
				...
				if($donateMethod == "pagseguro") {
                    ...
                    $main_content .= '<form action="?subtopic=account&action=donate" method="post" enctype="application/x-www-form-urlencoded"
				...
				$main_content .= '
					<TABLE BORDER=0 WIDTH=100%>
						<TR>
							<TD ALIGN=center>
								<table border="0" cellspacing="0" cellpadding="0" >
									<form action="?subtopic=account&action=manage" method="post">
										<tr>
											<td style="border:0px;" ><div class="BigButton" style="background-image:url('.$layout_name.'/images/global/buttons/sbutton.gif)" >
													<div onMouseOver="MouseOverBigButton(this);" onMouseOut="MouseOutBigButton(this);" ><div class="BigButtonOver" style="background-image:url('.$layout_name.'/images/global/buttons/sbutton_over.gif);" ></div>
														<input class="ButtonText" type="image" name="Back" alt="Back" src="'.$layout_name.'/images/global/buttons/_sbutton_back.gif" >
