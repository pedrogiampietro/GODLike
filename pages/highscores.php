...
$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Ranking for '.htmlspecialchars($list_name).'</h3></div><div class="panel-body"><div class="text-center"><div class="btn-group">';
	foreach ($tmpSkills as $k => $v) {
		$main_content .= '<a href="?subtopic=highscores&skill='.$k.'" class="btn btn-default '.($list == $k ? 'active' : '') .'">'.$v.'</a>';
	}
...
foreach($skills as $skill) {
	...
	$main_content .= '<tr><td>'.($offset + $number_of_rows).'</td><td><a href="?subtopic=characters&name='.urlencode($skill->getName()).'">'.htmlspecialchars($skill->getName()).'</a></td><td>'.htmlspecialchars(Website::getVocationName($skill->getVocation())).'</td><td>'.$value.'</td></tr>';
}

$main_content .= '</tbody></table>';

$main_content .= '<center><div class="text-center"><ul class="pagination">';
$pages = (int)($rows / $rowsPerPage);
for ($i = 0; $i < $pages; $i++) {
	$x = $i + 1;
	$main_content .= '<li '. (($x - 1) == $page ? 'class="active"' : '') .'><a href="?subtopic=highscores&skill='.urlencode($list).'&page='.($x - 1).'" data-original-title="" title="">'.($x).'</a></li>';
}

$main_content .= '</ul></div></center>';
