...
if(!$logged)
	$main_content .=  '<div class="alert alert-danger">You are not logged in. <a class="alert-link" href="?subtopic=account">Log in</a> to post on the forum.</div>';
			...
			<div class="panel-body">
				<table class="table table-striped table-condensed">
					...
					foreach($sections as $id => $section)
					{
						...
						$main_content .= '<tr><td><a href="?subtopic=forum&action=show_board&id='.$id.'">'.$section.'</a> - <small>'.$sections_desc[$id].'</small></td><td>'.(int) $counters[$id]['posts'].'</td><td>'.(int) $counters[$id]['threads'].'</td><td>';
						if(isset($last_post['name']))
							$main_content .= date('d.m.y H:i:s', $last_post['post_date']).' by <a href="?subtopic=characters&name='.urlencode($last_post['name']).'">'.$last_post['name'].'</a>';
						else
							$main_content .= '...';
						$main_content .= '</td></tr>';
					}
...
if($action == 'show_board')
{
	...
	$main_content .= '
		<div class="panel panel-default">
			<div class="panel-heading">';
				if ($logged) {
					$main_content .= '<a class="btn btn-sm btn-success pull-right" style="top:4px;position:relative;" href="?subtopic=forum&action=new_topic&section_id='.$section_id.'"><i class="fa fa-plus"></i> New Topic</a>';
				}
				$main_content .= '<h3 class="panel-title"><a href="?subtopic=forum">Boards</a> / <b>'.$sections[$section_id].'</b></h3>
			</div>
			<div class="panel-body">
				<table class="table table-striped table-condensed">
					<thead>
						...
						if (isset($last_threads[0])) {
							foreach ($last_threads as $thread) {
								...
								if ($logged && $group_id_of_acc_logged >= $group_not_blocked)
									$main_content .= '<a class="btn btn-xs btn-danger" href="?subtopic=forum&action=remove_post&id='.$thread['id'].'" onclick="return confirm(\'Are you sure you want remove thread > '.htmlspecialchars($thread['post_topic']).' <?\')"><i class="fa fa-close"></i> Remove</a> ';
								$main_content .= '<a href="?subtopic=forum&action=show_thread&id='.$thread['id'].'">'.htmlspecialchars($thread['post_topic']).'</a></td><td><a href="?subtopic=characters&name='.urlencode($thread['name']).'">'.$thread['name'].'</a></td><td>'.(int) $thread['replies'].'</td><td>'.(int) $thread['views'].'</td><td>';
								...
								if ($thread['last_post'] > 0) {
									...
									$main_content .= date('d.m.y H:i:s', $thread['post_date']).' by <a href="?subtopic=characters&name='.urlencode($thread['name']).'">'.$thread['name'].'</a>';
								$main_content .= '</td></tr>';
							}
						} else {
...
if($action == 'show_thread')
{
	...
	if(!empty($thread_name['name']))
	{
		...
		<div class="panel panel-default">
			<div class="panel-heading">';
				if ($logged) {
					$main_content .= '<a class="btn btn-sm btn-success pull-right" style="top:4px;position:relative;" href="?subtopic=forum&action=new_post&thread_id='.$thread_id.'"><i class="fa fa-plus"></i> Post Reply</a>';
				}
				$main_content .= '<h3 class="panel-title"><a href="?subtopic=forum">Boards</a> / <a href="?subtopic=forum&action=show_board&id='.$threads[0]['section'].'">'.$sections[$threads[0]['section']].'</a> / <b>'.htmlspecialchars($thread_name['post_topic']).'</b></h3>
			</div>
			<div class="panel-body">
				<ul class="chat">';
					$number_of_rows = 0;
						...
						$main_content .= '
							<li class="left clearfix">
								...
								<div class="chat-body clearfix">
									<strong class="primary-font"><a href="?subtopic=characters&name='.urlencode($thread['name']).'">'.htmlspecialchars($thread['name']).'</a></strong>
									<small class="pull-right text-muted">';
									if ($logged && $group_id_of_acc_logged >= $group_not_blocked)
										if($thread['first_post'] != $thread['id'])
											$main_content .= '<a class="btn btn-xs btn-danger" href="?subtopic=forum&action=remove_post&id='.$thread['id'].'" onclick="return confirm(\'Are you sure you want remove post of '.htmlspecialchars($thread['name']).'?\')"><i class="fa fa-close"></i> Remove</a>';
										else
											$main_content .= '<a class="btn btn-xs btn-danger" href="?subtopic=forum&action=remove_post&id='.$thread['id'].'" onclick="return confirm(\'Are you sure you want remove thread > '.htmlspecialchars($thread['post_topic']).' <?\')"><i class="fa fa-close"></i> Remove</a>';
									if ($logged && ($thread['account_id'] == $account_logged->getId() || $group_id_of_acc_logged >= $group_not_blocked))
										$main_content .= '<a class="btn btn-xs btn-success" style="margin-left:5px;margin-right:5px;" href="?subtopic=forum&action=edit_post&id='.$thread['id'].'"><i class="fa fa-pencil"></i> Edit</a>';
									if ($logged)
										$main_content .= '<a class="btn btn-xs btn-info" style="margin-right:5px;" href="?subtopic=forum&action=new_post&thread_id='.$thread_id.'&quote='.$thread['id'].'"><i class="fa fa-quote-left"></i> Quote</a>';
		...
		<div class="panel panel-default">
			<div class="panel-heading">';
				if ($logged) {
					$main_content .= '<a class="btn btn-sm btn-success pull-right" style="top:-7px;position:relative;" href="?subtopic=forum&action=new_post&thread_id='.$thread_id.'"><i class="fa fa-plus"></i> Post Reply</a>';
				}
				$main_content .= '<h3 class="panel-title"><a href="?subtopic=forum">Boards</a> / <a href="?subtopic=forum&action=show_board&id='.$threads[0]['section'].'">'.$sections[$threads[0]['section']].'</a> / <b>'.htmlspecialchars($thread_name['post_topic']).'</b></h3>
			</div>
			<div class="panel-body">';
				foreach($threads as $thread)
				{
					...
					$main_content .= '<tr><td width="200" valign="top"><a href="?subtopic=characters&name='.urlencode($thread['name']).'">'.htmlspecialchars($thread['name']).'</a><br /><br /><font size="1">Profession: '.htmlspecialchars(Website::getVocationName($thread['vocation'])).'<br />Level: '.$thread['level'].'<br />';
					if($thread['rank_id'] > 0)
					{
			...
			if(isset($thread['id']))
			{
				...
				if(!$saved)
				{
					...
					$main_content .= '
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3 class="panel-title"><a href="?subtopic=forum">Boards</a> / <a href="?subtopic=forum&action=show_board&id='.$thread['section'].'">'.$sections[$thread['section']].'</a> / <a href="?subtopic=forum&action=show_thread&id='.$thread_id.'">'.htmlspecialchars($thread['post_topic']).'</a> / <b>Post new reply</b></h3>
							</div>
							<div class="panel-body">
								<form class="form-horizontal" role="form" action="?" method="post">
									...
									<fieldset>
										...
										<div class="text-center">
											...
											<a class="btn btn-default" href="?subtopic=forum&action=show_thread&id='.$thread_id.'">Back</a>
										</div>
					...
					if(!$saved)
					{
						if(count($errors) > 0)
						{
							...
							$main_content .= '
								<div class="panel panel-default">
									<div class="panel-heading">
										<h3 class="panel-title"><a href="?subtopic=forum">Boards</a> / <a href="?subtopic=forum&action=show_board&id='.$thread['section'].'">'.$sections[$thread['section']].'</a> / <a href="?subtopic=forum&action=show_thread&id='.$thread['first_post'].'">'.htmlspecialchars($first_post['post_topic']).'</a> / <b>Edit post</b></h3>
									</div>
									<div class="panel-body">
										<form class="form-horizontal" role="form" action="?" method="post">
											...
											<fieldset>
												...
												<div class="text-center">
													...
													<a class="btn btn-default" href="?subtopic=forum&action=show_thread&id='.$thread['first_post'].'">Back</a>
												</div>
			...
			if(isset($sections[$section_id]))
			{
				...
				if(!$saved)
				{
					...
					$main_content .= '
						<div class="panel panel-default">
							<div class="panel-heading">
								<h3 class="panel-title"><a href="?subtopic=forum">Boards</a> / <a href="?subtopic=forum&action=show_board&id='.$section_id.'">'.$sections[$section_id].'</a> / <b>Post new thread</b></h3>
							</div>
							<div class="panel-body">
								<form class="form-horizontal" role="form" action="?" method="post">
									...
									<fieldset>
										...
										<div class="text-center">
											<button type="submit" class="btn btn-primary">Submit</button>
											<a class="btn btn-default" href="?subtopic=forum&action=show_board&id='.$section_id.'">Back</a>
										</div>
