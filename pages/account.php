...
if(!$logged)
	...
	else
	{
		...
		$main_content .= '
				...
				<p><a href="?subtopic=register" >Create an account</a> if you do not have one yet.</p><br>

						<form class="form-horizontal" role="form" action="?subtopic=account" method="post">
						
                            <input type="password" class="form-control required" id="account_login" name="account_login" placeholder="Account Name *">
                            <div class="nk-gap"></div>

                            <input type="password" class="form-control required" id="password_login" name="password_login" placeholder="Password *">
                            <div class="nk-gap"></div>

                            <div class="nk-form-response-success"></div>
                            <div class="nk-form-response-error"></div>
                            <button type="submit" class="nk-btn nk-btn-circle nk-btn-color-main-1">Submit</button>
							<a class="nk-btn nk-btn-circle nk-btn-color-main-5" href="?view=lostaccount">Account Lost?</a>
...
else
{
	if($action == "")
	{
		...
		<a class="nk-btn nk-btn-circle nk-btn-color-main-5 pull-right" style="position:relative;top: 4px;" href="?subtopic=account&action=logout"><i class="fa fa-sign-out"></i> Logout</a>
		...
		$main_content .= '<div class="panel panel-default">
			...
			<table class="table table-striped table-condensed table-bordered">
				...
				<tr>
					<td colspan="2">
						<a href="?subtopic=account&action=changepassword" class="nk-btn nk-btn-circle nk-btn nk-btn-color-info"><i class="fa fa-lock"></i> Change Password</a>
						<a href="?subtopic=account&action=changeemail" class="nk-btn nk-btn-circle nk-btn nk-btn-color-info"><i class="fa fa-pencil-square-o"></i> Change Email</a>
						'.(!$rec_key ? '<a href="?subtopic=account&action=generatekey" class="nk-btn nk-btn-circle nk-btn nk-btn-color-info"><i class="fa fa-key"></i> Generate Recovery Key</a>' : '').'
						<a href="?subtopic=purchasecoins" class="nk-btn nk-btn-circle nk-btn-color-main-1 pull-right"><i class="fa fa-cart-plus fa-lg"></i> Shop</a>
					</td>
				</tr>
			</table>
		</div>';
								...
								<div class="InnerTableContainer">
										<table style="width:100%;">
											<tbody><tr>
                                                ...
                                                $main_content .= '
                                                            <tr bgcolor="#999999">                                                                
                                                                <td><a href="?subtopic=account&action=showtickets"><small>Show All</small></a></td>
		...
		<div class="btn-group pull-right" style="top:-5px;position:relative;">
			<a class="nk-btn nk-btn-circle nk-btn-color-main-2" style="margin-right: 5px;" href="?subtopic=account&action=createcharacter"><i class="fa fa-plus"></i> Create a new character</a>
			<a class="nk-btn nk-btn-circle nk-btn-color-main-5" style="margin-top: 1px;" href="?subtopic=account&action=deletecharacter"><i class="fa fa-trash"></i> Delete a character</a>
		</div>
		...
		foreach ($account_players as $account_player) {
			...
			if ($account_player->isDeleted()) {
				$main_content .= '<strike><a data-toggle="tooltip" data-placement="right" data-original-title="Deleted" href="?subtopic=characters&name='.urlencode($account_player->getName()).'">'.htmlspecialchars($account_player->getName()).'</a></strike><div class="pull-right"><a class="btn btn-sm btn-warning" style="margin-right: 5px;" href="?subtopic=account&action=undelete&name='.urlencode($account_player->getName()).'"><i class="fa fa-undo"></i> Undelete</a>';
			} else {
				$main_content .= '<a href="?subtopic=characters&name='.urlencode($account_player->getName()).'">'.htmlspecialchars($account_player->getName()).'</a><div class="pull-right">';
			}
			$main_content .= '<a style="margin-top:-px;" class="btn btn-sm btn-primary" href="?subtopic=account&action=editcharacter&name='.urlencode($account_player->getName()).'"><i class="fa fa-pencil"></i> Edit</a></div></td></tr>';
		}
	...
	if ($action == "changepassword") {
		...
		if (empty($new_password) && empty($new_password2) && empty($old_password)) {
			...
			$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Change Password</h3></div><div class="panel-body">
				...
				<form class="form-horizontal" role="form" action="?subtopic=account&action=changepassword" method="post">
					<fieldset>
						...
						<div class="text-center">
							<button type="submit" class="nk-btn nk-btn-color-primary">Submit</button>
			...
			if (!empty($show_msgs)) {
				...
				<form class="form-horizontal" role="form" action="?subtopic=account&action=changepassword" method="post">
					<fieldset>
						...
						<div class="text-center">
							...
							<a href="?subtopic=account" class="btn btn-default">Back</a>
						</div>
				...
				$account_logged->save();
				$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Password Changed</h3></div><div class="panel-body"><p>Your password has been changed successfully.</p><p><a href="?subtopic=account" class="btn btn-primary form-control">Back</a></p></div></div>';
...
//############# CHANGE E-MAIL ###################
	if($action == "changeemail")
	{
		...
		if($account_email_new_time < 10)
		{
			if($_POST['changeemailsave'] == 1)
			{
				...
				else
				{
					...
					$main_content .= '<form class="form-horizontal" role="form" action="?subtopic=account&action=changeemail" method="post">
						<fieldset>
		...
		else
		{
			if($account_email_new_time < time())
			{
				if($_POST['changeemailsave'] == 1)
				{
					...
					$account_logged->save();
					$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Email Address Changed</h3></div><div class="panel-body"><p>You have accepted <b>'.htmlspecialchars($account_logged->getEmail()).'</b> as your new email address.</p><div class="text-center"><a href="?subtopic=account" class="btn btn-primary">Back</a></div></div></div>';
				}
				else
				{
		...
		if($_POST['emailchangecancel'] == 1)
		{
			...
			$account_logged->set("email_new_time", 0);
			$account_logged->save();
			$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Email Address Change Cancelled</h3></div><div class="panel-body"><p>Your request to change the email address of your account has been cancelled. The email address will not be changed.</p><div class="text-center"><a href="?subtopic=account" class="btn btn-primary">Back</a></div></div></div>';
		}
	}
...
//############## GENERATE RECOVERY KEY ###########
	if($action == "generatekey")
	{
		...
		if($dontshowtableagain != 1)
		{
			...
			$main_content .= '<form class="form-horizontal" role="form" action="?subtopic=account&action=generatekey" method="post"><input type="hidden" name="generatekeysave" value="1">
					<fieldset>
						<div class="form-group">
			...
			if($player->isLoaded())
			{
				...
				if($account_logged->getId() == $player_account->getId())
				{
					if($_POST['editcharactersave'] == 1)
					{
						...
						$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Character Information Changed</h3></div><div class="panel-body"><p>The character information has been changed.</p><div class="text-center"><a href="?subtopic=account" class="btn btn-primary">Back</a></div></div></div>';
					} else {
						$main_content .= '<div class="nk-box bg-dark-1 text-xs-center">
						<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Edit Character Information</h3></div><div class="panel-body">Here you can see and edit the information about your character.<p>If you do not want to specify a certain field, just leave it blank.</p>';

						$main_content .= '<form class="form-horizontal" role="form" action="?subtopic=account&action=editcharacter" method="post">
...
//### DELETE character from account ###
	if($action == "deletecharacter")
	{
						...
						if($account_logged->getId() == $player_account->getId())
						{
							if($account_logged->isValidPassword($password_verify))
							{
								if(!$player->isOnline())
								{
									...
									$player->save();
									$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Character Deleted</h3></div><div class="panel-body"><p>The character '.htmlspecialchars($player_name).' has been deleted.</p><div class="text-center"><a class="btn btn-primary" href="?subtopic=account">Back</a></div></div></div>';
								}
								else
		...
		if($dontshowtableagain != 1)
		{
			...
			$main_content .= '<form class="form-horizontal" role="form" action="?subtopic=account&action=deletecharacter" method="post"><input type="hidden" name="deletecharactersave" value="1">
					<fieldset>
...
//### UNDELETE character from account ###
				...
				if($player->isLoaded())
				{
					...
					if($account_logged->getId() == $player_account->getId())
					{
						if(!$player->isOnline())
						{
							...
							$player->save();
							$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Character Undeleted</h3></div><div class="panel-body"><p>The character <b>'.htmlspecialchars($player_name).'</b> has been undeleted.</p><div class="text-center"><a href="?subtopic=account" class="btn btn-primary">Back</a></div></div></div>';
							//$main_content .= '<div class="TableContainer" >  <table class="Table1" cellpadding="0" cellspacing="0" >    <div class="CaptionContainer" >      <div class="CaptionInnerContainer" >        <span class="CaptionEdgeLeftTop" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>        <span class="CaptionEdgeRightTop" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>        <span class="CaptionBorderTop" style="background-image:url('.$layout_name.'/images/content/table-headline-border.gif);" ></span>        <span class="CaptionVerticalLeft" style="background-image:url('.$layout_name.'/images/content/box-frame-vertical.gif);" /></span>        <div class="Text" >Character Undeleted</div>        <span class="CaptionVerticalRight" style="background-image:url('.$layout_name.'/images/content/box-frame-vertical.gif);" /></span>        <span class="CaptionBorderBottom" style="background-image:url('.$layout_name.'/images/content/table-headline-border.gif);" ></span>        <span class="CaptionEdgeLeftBottom" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>        <span class="CaptionEdgeRightBottom" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>      </div>    </div>    <tr>      <td>        <div class="InnerTableContainer" >          <table style="width:100%;" ><tr><td>The character <b>'.htmlspecialchars($player_name).'</b> has been undeleted.</td></tr>          </table>        </div>  </table></div></td></tr><br><center><table border="0" cellspacing="0" cellpadding="0" ><form action="?subtopic=account" method="post" ><tr><td style="border:0px;" ><div class="BigButton" style="background-image:url('.$layout_name.'/images/buttons/sbutton.gif)" ><div onMouseOver="MouseOverBigButton(this);" onMouseOut="MouseOutBigButton(this);" ><div class="BigButtonOver" style="background-image:url('.$layout_name.'/images/buttons/sbutton_over.gif);" ></div><input class="ButtonText" type="image" name="Back" alt="Back" src="'.$layout_name.'/images/buttons/_sbutton_back.gif" ></div></div></td></tr></form></table></center>';
						}
						else
							$delete_errors[] = 'This character is online.';
					}
	...
	if ($action == "createcharacter")
	{
		if (isset($_POST['step']) && $_POST['step'] == 'create') {
			...
			$main_content .= '<div class="nk-doc nk-box-4 bg-dark-1">
			<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Create Character</h3></div><div class="panel-body">
				<p>Please choose a name, vocation and sex for your character. <br/>In any case the name must not violate the naming conventions stated in the <a href="?subtopic=rules" target="_blank" >Burmourne Rules</a>, or your character might get deleted or name locked.</p>
				<form class="form-horizontal" role="form" action="?subtopic=account&action=createcharacter" method="post">
					<fieldset>
